...
def make_rect_roi_mask(H, W, x1, y1, x2, y2):
    """
    生成矩形 ROI 掩膜。
    ROI 内为 255，ROI 外为 0。
    后续所有植物检测都只在这个 ROI 内进行。
    """
...
def main():
    ...
    cv2.resizeWindow("controls", 600, 760)
    ...
    # ========= 分类参数 =========
    cv2.createTrackbar("Aspect_Jiu_x10", "controls", 30, 150, nothing)
    cv2.createTrackbar("TipCmp_Tong", "controls", 40, 300, nothing)
    cv2.createTrackbar("TipWidthRatio_x10", "controls", 18, 100, nothing)
    ...
    # ========= 新增：ROI区域参数 =========
    # 只在蓝色板区域内检测。默认值需要你根据实际画面微调。
    cv2.createTrackbar("ROI_X1", "controls", 300, 1280, nothing)
    cv2.createTrackbar("ROI_Y1", "controls", 90, 720, nothing)
    cv2.createTrackbar("ROI_X2", "controls", 700, 1280, nothing)
    cv2.createTrackbar("ROI_Y2", "controls", 400, 720, nothing)
    ...
    while True:
        ...
        # ===== 读取 ROI 参数 =====
        roi_x1 = cv2.getTrackbarPos("ROI_X1", "controls")
        roi_y1 = cv2.getTrackbarPos("ROI_Y1", "controls")
        roi_x2 = cv2.getTrackbarPos("ROI_X2", "controls")
        roi_y2 = cv2.getTrackbarPos("ROI_Y2", "controls")
        roi_mask, roi_rect = make_rect_roi_mask(H, W, roi_x1, roi_y1, roi_x2, roi_y2)
        roi_x1, roi_y1, roi_x2, roi_y2 = roi_rect

        # ===== 1. 语义分割 =====

        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        blurred_1 = cv2.GaussianBlur(gray, (gk, gk), 0)

        blurred = cv2.GaussianBlur(frame, (gk, gk), 0)
        #cv2.imshow("Gaussian Frame", blurred)
        ...
        #v2.imshow("Eroded Mask", eroded)


        # kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        # dilated = cv2.dilate(eroded, kernel, iterations=1)

        cv2.imshow("Before Dilate", eroded)
        ...
        # ===== 新增：只保留 ROI 内的植物前景 =====
        plant_mask = cv2.bitwise_and(plant_mask, roi_mask)
        ...
        # 形态学处理后再套一次 ROI，防止闭运算把 ROI 外区域连回来
        plant_mask = cv2.bitwise_and(plant_mask, roi_mask)
        ...
        # 腐蚀后再限制 ROI
        plant_mask_cls = cv2.bitwise_and(plant_mask_cls, roi_mask)
        ...
        # 显示 ROI 框：紫色框内才进行植物检测
        cv2.rectangle(vis, (roi_x1, roi_y1), (roi_x2, roi_y2), (255, 0, 255), 2)
        cv2.putText(
            vis,
            "ROI",
            (roi_x1 + 8, max(25, roi_y1 - 8)),
        ...
        cv2.rectangle(mask_bgr, (roi_x1, roi_y1), (roi_x2, roi_y2), (255, 0, 255), 2)
        ...
        for i in range(1, num_labels):
            ...
            if shown < 10:
                feature_lines.append(
                    f"{label} asp={feat.get('aspect_ratio', 0):.2f} "
                    f"tipCmp={feat.get('tip_complexity', 0):.1f} "
                    f"tipWR={feat.get('tip_width_ratio', 0):.2f}"
                )
                shown += 1
        ...
        # ===== 6. 信息栏 =====
        info_panel = np.zeros((290, W * 2, 3), dtype=np.uint8)
        ...
        info6 = f"ROI=({roi_x1},{roi_y1})-({roi_x2},{roi_y2})  Only detect inside ROI"
        ...
        cv2.putText(info_panel, info5, (20, 175),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2)
        cv2.putText(info_panel, info6, (20, 210),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 0, 255), 2)

        cv2.putText(info_panel, "JIUCAI=Green  TONGHAO=Orange  UNKNOWN=Gray  TIP=Yellow  ROI=Purple",
                    (20, 245), cv2.FONT_HERSHEY_SIMPLEX, 0.70, (255, 255, 255), 2)
        ...
        cv2.putText(final_show, "Left: Original + ROI + Classification", (20, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.72, (255, 255, 255), 2)
        cv2.putText(final_show, "Right: ROI-limited Semantic Mask after top-cut + erode", (W + 20, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.72, (255, 255, 255), 2)

        cv2.imshow("tongjiu2 morph v6 separated + ROI", final_show)

        key = cv2.waitKey(1) & 0xFF
        if key == ord("q") or key == 27:
            break
        elif key == ord("s"):
            cv2.imwrite("classified_view_roi.png", vis)
            cv2.imwrite("semantic_mask_cls_roi.png", plant_mask_cls)
            cv2.imwrite("info_panel_roi.png", info_panel)
            print("已保存: classified_view_roi.png / semantic_mask_cls_roi.png / info_panel_roi.png")
